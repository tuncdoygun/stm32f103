...
// Cevresel kisimdaki eventi aktif etti sadece.
// Bu durumda kesme hicbir zaman olusamaz.
// Bu fonksiyon kullanildiginde kesme olusmaz,yani IRQ_Handler'a girmez,sadece event olusturur.
void Timer_EvtConfig(int tmNo) // eventten kasit bir sinyal.Burda bir kesme kaynagini event olarak kullanacak.
{
  // 1) Cevresel birim ayarlari
  // a) False interrupt onlemi
  TIM_ClearITPendingBit(TimTab[tmNo], TIM_IT_Update);
  // b) cevresel yakada kesme kaynagini aktive ediyoruz
